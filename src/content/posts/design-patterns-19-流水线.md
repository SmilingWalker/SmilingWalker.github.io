...
订单接入的第一版只有三步：收单、算价、落库。一个函数五十行，清楚得像教科书示例，没有任何理由动它。大促改造时加了风控标记和拆单，函数长到两百行——开始不舒服，但每次改动都能对付。压垮判断的是 A/B 实验的需求：给实验组的订单插一步"赠品策略"，实验两周，到期下线。开发者盯着函数中段的位置想了想：插进去容易，两周后把这段代码再挖出来、确认没伤到旁边的步骤——这才是让人半夜睡不着的部分。而且往后看，这类"要来来、要走走"的步骤只会更多。
