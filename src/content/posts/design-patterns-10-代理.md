...
CloudShop 商品实体的现实：标题、价格、主图都是小字段，但详情富文本 200KB、视频预览 5MB。而买家在搜索结果页划过一屏 50 个商品时，绝大多数一次也不会点进详情页——`SELECT *` 没有这种判断力，50 个商品的富文本和视频被整页拖回来，250MB 直接把列表页 P99 顶爆。买家要的只是标题和价格，却为没人看的内容付出了全部加载成本。
