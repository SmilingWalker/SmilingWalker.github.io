...
[适配器](/posts/design-patterns-6-适配器/)解决"接口对不上"，这篇解决"接口太多"。

先看一笔订单成交的那几秒，CloudShop 内部要发生多少事：锁定库存防超卖、跑定价引擎算实付、核销用户勾选的券、创建订单、唤起支付。这些子系统分属不同团队，各自设计良好，单看谁都没病。病在调用方：前台有三端——App、小程序、H5——每端做"去结算"时，都得把这个流程从头到尾自己背一遍，因为系统里没有一个地方写明"下单到底要经过哪几步、按什么顺序"。这份流程知识只能长在每个端自己的代码里，而流程是会变的：大促要临时加一步限购检查，三端各改一遍，哪端漏了，哪端就成了绕过限购的口子。

子系统本身没病——库存、定价、核券、创单、支付每个都设计良好——但**正确使用它们的流程**，不该是每个调用方都背一遍的常识。
