...
类目树也不是生来就复杂。CloudShop 开张时类目就两层：一级类目直接挂商品，"汇总这个类目下的价格区间"就是一层循环，谁都会写。三级类目上线后，循环套了循环，代码开始递归——还行，递归也算清楚。真正的麻烦随"家电节专题"到来：专题可以横跨多个类目，专题底下还能再有子专题，子专题里既有商品也有小专题。树从此不再规则，而产品在类目运营后台的需求单上写的是：任意节点上显示"该节点下的价格区间和总库存"——点"家电"要看整棵子树汇总。
