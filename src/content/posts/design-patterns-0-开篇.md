...
出事之后，多数人的下一步是学设计模式：打开一篇教程，看到"策略模式：定义算法族，分别封装起来，让它们之间可以互相替换"，配一个会飞会叫的鸭子示例，背完 23 种定义，做好笔记。

三个月后，站在自己那个三层嵌套的 if-else 前，还是不知道从哪里下手。

背完定义不会用，是模式学习里被反复报告的现象。Reddit 的 r/developersIndia 有个[《学设计模式学到崩溃》](https://www.reddit.com/r/developersIndia/comments/1mfydex/frustrated_with_learning_all_of_the_design/)的帖子，跟帖上万，描述的几乎是同一种经历：定义背下来了，回到自己的代码前仍然无从下手。

问题出在呈现顺序上。多数教程的结构是**先答案，后问题**，甚至只有答案：一张标准 UML 类图、一段定义、一个鸭子或形状的示例。而模式属于程序性知识，需要问题情境作为提取线索——在没有经历对应问题的情况下接收答案，答案只能以孤立陈述的形式存储，缺少编码时的情境锚点，遗忘曲线陡峭。最后留存的，往往是"鸭子会飞"这类和自身经验无关的碎片。

教育学的结论与此一致：基于问题的学习（problem-based learning）在知识留存和迁移上都优于"先讲授后举例"的呈现。Martin Fowler 在[《Writing Software Patterns》](https://www.martinfowler.com/articles/writingPatterns.html)里把同一件事说成模式观：模式描述的是**情境中的力（forces）和取舍**，不是照抄的菜谱。没有感受过那个"力"，抄的就只是菜谱。

美团技术团队在[《设计模式在外卖营销业务中的实践》](https://tech.meituan.com/2020/03/19/Software-design-pattern-practice-in-marketing.html)里说得更直白："如果脱离具体的业务逻辑去学习或者使用设计模式，那是极其空洞的。"他们在营销业务里用工厂+策略算邀请返奖、用责任链做投放资源位过滤——大厂给"业务级学模式"这条路线背了书。

所以这个系列把顺序倒过来：**先疼，再重构，后命名**。每篇从一段真实会写出来的烂代码开始，小步重构，直到结构自己长成模式的形状——那一刻等于重新发明了它，命名只是补发身份证。这条路线出自 Joshua Kerievsky 的《Refactoring to Patterns》。
...
多数教程只送到第 2 层，面试考的也是第 2 层。但决定促销互斥那晚敢不敢动手的，是第 4 层；能把那段"别动这里"一步步拆开重来的，是第 5 层。
...
单一职责最容易问的是"多少算多"。王争在《设计模式之美》给了五条侧面判据，比"凭感觉"可执行得多：类中代码行数、函数数、属性过多（经验参考值：类 200 行上下、函数和属性各 10 个）；它依赖的类、或依赖它的类过多；私有方法过多（该独立成类提高复用）；**给类起不出一个贴切的名字**，只能叫 XxxManager、XxxContext；大量方法总是集中操作某几个属性。占了两条以上，就值得拆——但方向反过来同样成立：拆得过细会反噬内聚（协议一改要同步改两个类），先写粗粒度、疼了再拆是正路。
...
违背的方式有三种，王争的清单值得照着自查：子类违背父类声明的功能（父类按金额排序、子类改成按日期）；违背输入输出异常的约定（父类出错返回 null、子类抛异常；子类对输入校验比父类更严）；违背注释里的特殊说明（父类注释"提现不得超过余额"、子类给 VIP 开了透支）。前两种编译器不拦，最阴险的检验窍门是一条：**拿父类的单元测试去跑子类**，跑不过的就是毁约。
...
五条原则，五种疼。后面的篇章会反复回到这五种疼——它们比原则的名字好记，也更能解释每个模式为什么长那样。
...
三条学习建议。代码自己敲一遍——阅读时能跳过的细节，动手时都会暴露。自测先做再看答案，做错的那道第二天重做，间隔重测的留存效果在认知研究里结论一致。每篇隔一周回看要点，能说出"什么时候不用它"，才算到第 4 层。
