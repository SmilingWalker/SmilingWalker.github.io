...
[第 6 篇](/posts/design-patterns-6-适配器/)把三家支付渠道的差异关进了适配器。但适配器只管"接口翻译"，还有一个更隐蔽的疼没治：**流程骨架的拷贝**。

一笔支付从进门到落库，业务上要走一段固定的序列：验参、幂等检查、构造渠道请求、调用渠道、错误映射、解析响应、落库。序列本身对所有渠道一视同仁——幂等和记账不关心这一单走的是微信还是银联。但 CloudShop 的三次渠道接入发生在三个时期、经手三个工程师，每人对着手里那份渠道文档，从第一行写到最后一行——七步序列被原样写了三遍。它藏得很深：三家各自接完、各自跑通，谁也没觉得有问题，直到一次跨渠道的代码评审把三个文件并排放在了大屏上。
...
v2 那种"骨架函数收步骤函数"的形态，在模式谱系里叫**回调（Callback）**。王争的辨析值得整段吸收：回调与模板方法**作用相同**（复用+扩展），差别全在实现——模板基于继承（钩子是子类的方法），回调基于组合（钩子是被传入的函数），而组合的灵活有三层：不受单继承名额限制、无需为每个钩子预定义类、有多个钩子时只注入用到的（继承体系被迫实现全部抽象方法）。Go 没有继承，所以"Go 把模板方法翻译成钩子"的更准确说法是：**Go 天然只能走回调形态**——v2 不是妥协，是这个模式在 Go 的本体。

顺手记一个名字陷阱，来自 Spring 源码的一桩公案：`JdbcTemplate` 名字带 Template，实现却不是模板方法（没有继承骨架），而是标准的**同步回调**——不变的流程在 `execute()` 里，可变的 `doInStatement` 由使用者作为参数传入。判据一句话：**骨架住在父类里是模板，住在参数里是回调**——名字会骗人（第 24 篇还有更多），结构不会。
...
- 王争《设计模式之美》（模板/回调篇）— 回调基于组合优于继承的三层理由、JdbcTemplate 实为回调的澄清
