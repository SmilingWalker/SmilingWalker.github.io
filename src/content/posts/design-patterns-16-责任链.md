...
CloudShop 的下单风控，最初只有一条规则：黑名单。风控分析师手工维护一份名单，命中就拒单——一个 if 的事，平稳跑了两个月。刷子进场，限购规则上线；黄牛来了，评分模型接入；一单大额欺诈的案例复盘之后，"超过一万转人工"也加了进来。每条规则都是对真实伤害的正当回应，但 `CheckRisk` 函数跟着一条条长胖。真正让团队警觉的不是行数，是节奏：黑产的打法按周变，大促前风控要临时下掉评分规则（误杀率太高）、黑产散了又要加回来——**改代码、发版、回归的速度，跟不上风控本身的变化速度**。规则需要的是增删自如、顺序可调，而不是每次都动手术刀。
