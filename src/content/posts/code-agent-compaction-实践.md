...
Cursor 的做法跟 Claude Code / Codex 不太一样。Claude Code 追求"压缩时复用 cache、多层递进"，Codex 追求"三路分派、world_state 重建"，Cursor 的思路是"用便宜模型做摘要 + 存数据库 + 动态发现工具"。
...
这是 2026 年的一个 important 发现。来自论文 *Governance Decay: How Context Compaction Silently Erases Safety Constraints in Long-Horizon LLM Agents*（arXiv:2606.22528）。
