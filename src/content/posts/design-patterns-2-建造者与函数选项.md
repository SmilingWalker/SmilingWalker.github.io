...
[第 1 篇](/posts/design-patterns-1-策略模式/)解决了"算法怎么换"，这篇解决另一个每天都在发生的疼：**构造参数太多**。它不是一天长出来的。

CloudShop 的商品中台跑起来头三个月，查询接口只有一个用途：C 端搜索页按商家捞商品。那时 `NewProductQuery(merchantID, categoryID)` 两个参数，清清爽爽，谁看了都不会多想一眼。

变化从运营侧一点点渗进来。先是运营在群里问了句"大促前想盘一下在售商品，能不能按价格筛？"——加两个参数 `minPrice, maxPrice`，五分钟的活。第二周，仓库的人说导库存要"只看有货的"——再加一个 bool。又过了半个月，财务要核已删除商品的台账，`withDeleted` 上了签名。每一次都是"小需求顺手加一下"，没有一次值得专门开个设计讨论。

三个月后的一个下午，报表组的完整需求到了：导出运营报表，筛选维度要齐——商家、状态、价格区间、类目、页码页大小、排序、是否含已删除、是否只看有货、是否只看 SKU。开发者回到那个函数前，看着已经长到九个的参数列表，意识到要在这上面再摞三个。没有人做错什么，但函数签名已经变成了没人敢动的样子。
